...
class RetweetStreamTest < MiniTest::Unit::TestCase
  ...
  def test_update_original_tweets_add
    retweet_window = RetweetStream.new
    retweet_window.update_original_tweets("123", 1)
    retweet_window.update_original_tweets("123", 1)
    retweet_window.update_original_tweets("123", 1)

    assert_equal 3, retweet_window.original_tweets["123"]
  end

  def test_update_original_tweets_subtract
    retweet_window = RetweetStream.new
    retweet_window.update_original_tweets("123", 1)
    retweet_window.update_original_tweets("123", -1)
    
    assert_equal 0, retweet_window.original_tweets["123"]
  end
  ...
  def test_find_top_ten
    ...
    retweet_window.retweets << [10, 9, Time.now-15*60]
    retweet_window.update_original_tweets(9, 1)
    retweet_window.retweets << [13, 9, Time.now-8*60]
    retweet_window.update_original_tweets(9, 1)
    retweet_window.retweets << [18, 11, Time.now-8*60]
    retweet_window.update_original_tweets(11, 1)
    retweet_window.retweets << [20, 11, Time.now-7*60]
    retweet_window.update_original_tweets(11, 1)
    retweet_window.retweets << [25, 9, Time.now-6*60]
    retweet_window.update_original_tweets(9, 1)
    ...
    assert_equal 4, retweet_window.find_top_ten[0][1] #retweets count
  end
